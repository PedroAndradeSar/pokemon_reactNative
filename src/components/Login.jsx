...
const Login = ({ navigation }) => {
    ...
    const validateEmail = (text) => {
        ...
        return emailPattern.test(text) && text.length >= 5; // Aqui tem que ter mínimo de 5 caracteres para o email
    };

    const handleLogin = () => {
        if (isValidEmail && senha.length >= 6) {
            if (email === usuariosEstaticos.email && senha === usuariosEstaticos.senha) {
                // Aqui faz Login bem-sucedido, navegue para a página 'Home'
                navigation.navigate('Pokemon');

                setEmail('');
                setSenha('');
            } else {
                // Aqui exibe uma mensagem de erro, pois os dados estão incorretos
                alert('Email ou senha incorretos');
            }
        } else {
            // Exibe uma mensagem de erro se os campos não atenderem aos critérios
            alert('Email ou senha incorretos');
        }
    };
